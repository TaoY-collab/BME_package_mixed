...
def random_offset_center(
    center_dhw: Sequence[int],
    radius_vox: int,
    shape_dhw: Sequence[int],
    rng: random.Random,
) -> Tuple[int, int, int]:
    """Sample a shifted center around the nodule center."""
    radius = max(1, int(radius_vox))
    offset = np.asarray([rng.randint(-radius, radius) for _ in range(3)], dtype=int)
    center = np.asarray(center_dhw, dtype=int) + offset
...
def random_boundary_center(
    ...
    coords = boundary_voxels_dhw(mask_dhw)
    if coords.size == 0:
        return None
...
def append_failed_case(
    ...
    with failed_file.open("a", encoding="utf-8") as f:
        ...
        f.write(f"error_message: {str(error)}\n")
        f.write("traceback:\n")
...
def save_sampled_patch(
    ...
    if require_positive and accept_sum <= 0:
        return None
    ...
    mask_4d = ensure_channel_first_4d(mask_patch, dtype=np.float32)
    sdf_4d = ensure_channel_first_4d(sdf_patch, dtype=np.float32)
    boundary_4d = ensure_channel_first_4d(boundary_patch, dtype=np.float32)
    core_4d = ensure_channel_first_4d(core_patch, dtype=np.float32)
...
def process_one_scan(
    ...
    for item in nodule_items:
        ...
        try:
            random_saved = 0
            random_attempts = max(10, int(plan["random_positive"]) * 8)
            for _ in range(random_attempts):
                if random_saved >= int(plan["random_positive"]):
                    break
                shifted_center = random_offset_center(
                    center_dhw=center_dhw,
                    radius_vox=offset_radius_for_diameter(diameter_mm),
            ...
            negative_saved = 0
            negative_attempts = max(1, int(plan["hard_negative"]))
