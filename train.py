...
def parse_train_mode(argv):
    mode = os.environ.get("BME_TRAIN_MODE", "3d").lower()
    cleaned = [argv[0]]
    idx = 1
    while idx < len(argv):
        arg = argv[idx]
        if arg == "--train-mode":
            if idx + 1 >= len(argv):
                raise ValueError("--train-mode requires one of: 2d, 3d, stage2b")
            mode = argv[idx + 1].lower()
            idx += 2
            continue
        if arg.startswith("--train-mode="):
...
def run_train_entrypoint():
    mode, cleaned_argv = parse_train_mode(sys.argv)
    sys.argv = cleaned_argv

    if mode in {"2d", "swinunet2d", "swin_unet_2d", "swin-unet-2d"}:
        import train_2d_swinunet

        print("[INFO] train.py dispatch | mode=2d | trainer=train_2d_swinunet")
        train_2d_swinunet.main()
        return

    if mode in {"3d", "swinunetr", "swin_unetr", "stage2", "stage2_refine"}:
        print("[INFO] train.py dispatch | mode=3d | trainer=SwinUNETR")
        main()
        return

    if mode in {"stage2b", "3d_stage2b", "swinunetr_stage2b"}:
        print("[INFO] train.py dispatch | mode=stage2b | trainer=SwinUNETR inverse-weighted refinement")
        train_stage2b()
        return

    raise ValueError(f"Unsupported train mode: {mode}. Use one of: 2d, 3d, stage2b")


if __name__ == "__main__":
    run_train_entrypoint()
